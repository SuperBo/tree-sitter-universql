...
*/
    ...
    _kw_from: _ => token(prec(1, KW.FROM)),
    ...
    escape_sequence: _ => token.immediate(/\\[rn\.]|\\r\\n/),
    ...
    type_name: $ => choice($.path_expr, KW.INTERVAL),
    array_type: $ => seq(reserve('array'), '<', $.type, '>'),
    struct_field: $ => seq(optional($.identifier), $.type),
    struct_field_list: $ => seq($.struct_field, repeat(seq(',', $.struct_field))),
    struct_type: $ => seq(reserve('struct'), '<', optional($.struct_field_list), '>'), 
    raw_type: $ => choice($.array_type, $.struct_type, $.type_name),
    type_param_field: $ => choice(
      $.integer_literal,
      $.boolean_literal,
      $.string_literal,
      $.bytes_literal,
    ...
    type_param: $ => seq('(', $.type_param_field, repeat(seq(',', $.type_param_field)), ')'),
    type: $ => seq($.raw_type, optional($.type_param), optional($.collate_clause)),
    ...
    logical_unary_expr: $ => prec(PREC.LOGICAL_NOT, seq(KW.NOT, $.expr)),
    logical_binary_expr: $ => choice(
      prec.left(PREC.LOGICAL_AND, seq($.expr, KW.AND, $.expr)),
      prec.left(PREC.LOGICAL_OR, seq($.expr, KW.OR, $.expr)),
    ),
    compare_expr: $ => choice(
      prec(PREC.COMPARE, seq($._expr_primary, optNot(), choice(KW.LIKE, KW.ILIKE), $._expr_primary)),
      prec(PREC.COMPARE, seq($._expr_primary, optNot(), reserve('similar'), KW.TO, $._expr_primary)),
      prec(PREC.COMPARE, seq($._expr_primary, KW.IS, optional(KW.NOT),
      ...
      prec(PREC.COMPARE, seq($._expr_primary, KW.IS, optNot(), optional($.unicode_normal_form), reserve('normalized'))),
    ),
    ...
    _at_time_zone: $ => seq(reserve('at'), reserve('time'), reserve('zone'), $._const_expr),
    ...
    func_app: $ => seq($.path_expr, '(', optional(choice(
      $.star,
    ...
    func_arg_list_variadic: $ => seq(
      $._func_arg_expr, repeat(seq(',', $._func_arg_expr)), optional(seq(',', $.func_arg_variadic))
    ),
    ...
    func_expr_common_subexpr: $ => choice(
      seq(reserve('collation'), KW.FOR, '(', $.expr, ')'),
       reserveMany(
        'current_date',
      ...
      $.extract_expr,
      $.substr_expr,
      seq(reserve('normalize'), '(', $.expr, optional(seq(',', $.unicode_normal_form)),')'),
      seq(reserve('overlay'), '(', $.overlay_list, ')'),
      seq(reserve('overlay'), '(', optional($.func_arg_list), ')'),
      seq(reserve('position'), '(', $._expr_primary, KW.IN, $._expr_primary, ')'),
      seq(reserve('treat'), '(', $.expr, $._kw_as, $.type, ')'),
      seq(reserve('trim'), '(', optional(reserveMany('both', 'leading', 'trailing')), $.trim_list, ')'),
      $.xml_expr,
    ),
    ...
    _format: $ => seq(reserve('format'), $.expr, optional($._at_time_zone)),
    cast_expr: $ => seq(
      reserveMany('cast', 'safe_cast'), '(', $.expr, $._kw_as, $.type, optional($._format), ')'
    ),
    extract_expr: $ => seq(
    ...
    xmltable_column_element: $ => choice(
      seq($.identifier, $.type_name, $.xmltable_column_option_list),
    ...
    array_constructor: $ => seq(optional($._array_construct_prefix), '[', optional($._expr_list), ']'),
    ...
    */
    ...
    select_spec: $ => seq(
      reserve('select'), optional($.set_quantifier),
    ...
    table_ref: $ =>  choice(
      ...
      seq(reserve('lateral'), $.select_in_parens, optional($._alias_clause)),
      $.joined_table,
    ...
    _where_clause: $ => seq(reserve('where'), $.expr),
